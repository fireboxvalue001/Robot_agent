...
第一部分新增“识别委托单”，用于上传 PDF/TXT 检测委托单并提取样品、检测项目、标准、报告要求和待确认项。结果会适配为现有 `semantic_params`，但不会在这一层生成操作链。只有检测项目命中 VD10 已登记能力时才会输出 `target_device=VD10`，其他项目要求重新进行仪器路由。

该模块参考 `robot_agent/意图识别模块_本地测试包`，原测试包保持不变。配置、接口和能力边界见 `docs/DOCUMENT_INTENT_INTEGRATION.md`。
